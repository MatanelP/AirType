...
impl Keyboard {
    ...
    // AirType patch: the Text Input Source (TSM/HIToolbox) APIs in the body below
    // assert they run on the main thread on modern macOS (>=14/15). rdev's listen
    // loop runs its CFRunLoop on a background thread, so calling them there
    // triggers `dispatch_assert_queue` -> SIGTRAP and crashes the app on the first
    // key event. We never use `Event::name`, so skip the lookup entirely.
